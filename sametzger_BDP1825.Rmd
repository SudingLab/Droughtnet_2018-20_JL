...
### Load packages, data, functions + some cleaning
```{r load packages + data + functions; some cleaning, echo = FALSE, include=FALSE}
# Packages
...
```{r composition metrics: Shannon Diversity, Richness, & Evennes}
#'
#' *Estimate composition metrics*
#' *1. Richness, Evenness, & Shannon Diversity* 
#' 
...
#' *Estimate composition metrics*
#' *2. Functional Groups* 
```{r composition metrics: C4, C3, Ann-Bi, Forbs}
#'
#' *2. Functional Groups* 
#' 
...
#' Permanova + NMDS
```{r Species composition analysis}
#'
#' **ANALYSIS: Species composition**
#'   
...
```{r Linear Mixed Models - div, even, rich}
#'  *Shannon Diversity*
#'  
...
div_precip_pred <- ggpredict(div_mod_precip, terms = c("total_precip [all]", "graze_trt", "year"))

#' plot LMM trend line and raw data 
fig_div_precip <- ggplot() +
  # Raw data points
  geom_point(data = structure_precip, 
             aes(x = total_precip, y = shan, col = rain_trt, shape = graze_trt),alpha = 0.3, size = 1.5) +
  geom_ribbon(data = as.data.frame(div_precip_pred),
              aes(x = x, ymin = conf.low, ymax = conf.high, group = group), alpha = 0.15) +
  geom_line(data = as.data.frame(div_precip_pred),
            aes(x = x, y = predicted, group = group), linewidth = 1) +
  scale_color_manual(values = c("#d7191c", "tan", "#2c7bb6"), labels = c("Drought", "Control", "Wet")) +
  scale_shape_manual(values = c(16, 17, 15), labels = c("Ungrazed", "Fall Graze", "Spring Graze")) +
...
#' using total precip cm
rich_mod_precip <- lmer(even ~ total_precip * graze_trt * year + (1|block) + (1|b_p), data = structure_precip) 
...
#'  --Annual-Biennials
#'  --Forbs
```{r Linear Mixed Models - C4, C3, ann-bi, forbs}
#'  
#'  * C4 grasses *
#'
...
#'
#' *Total Precipitation*
```{r figures using precipitation amounts}
#' 
...
#' deleted code
```{r - code delated from sections above but good to keeo around for now}

#' from shannon diversity linear mixed model section
#' 
#' Model summary
anova(div_mod_cat)
r2(div_mod_cat)
